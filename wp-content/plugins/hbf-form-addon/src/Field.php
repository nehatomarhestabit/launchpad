...
class Field {
	...
	public function select( $attrs ) {
		...
		foreach ( $attrs['options'] as $key => $option) {
			$selected = (isset( $attrs['selected'] ) && $key == $attrs['selected'] ) ? 'selected="selected"' : ''; 
	...
	public function radio( $attrs ) {
		...
		foreach ( $attrs['options'] as $key => $option) {
			...
			if(isset( $attrs['checked'] ) && $key == $attrs['checked'] ) {
	...
	public function checkbox( $attrs ) {
		...
		foreach ( $attrs['options'] as $key => $option) {
			...
			if(isset( $attrs['checked'] ) && in_array( $key, $attrs['checked'] ) ) {
				$field['checked'] = 'checked';
			} 
	...
	public function textarea( $attrs ) {
		...
		$field_markup = sprintf( '<textarea %s>%s</textarea>', esc_attrs($field), $value );
	...
	public function file( $attrs ) {
		...
		$field_markup = sprintf( '<input %s /> %s', esc_attrs($field), $preview_block );
